...
append_upload_script() {
  cat $1 | awk -v PRESTON_REMOTE=$PRESTON_REMOTE -v HDFS_TARGET=$HDFS_TARGET '{ print "echo " $1 "| java -jar preston.jar get --remote " PRESTON_REMOTE " --no-cache | hdfs dfs -put -p - " HDFS_TARGET "/" $2 "/" substr($1, 15, 2) "/" substr($1, 17, 2) "/" substr($1, 15, 65) >> upload.sh
}
