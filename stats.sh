...
find data -type f | parallel --citation ls -s --block-size=1 {1} | pv -l | awk -F ' ' '{ print "hash://sha256/" substr($2,12) "\t" $1 }' | sort > hash_sizes.tsv
...
preston history -l tsv | tr '\t' '\n' | grep hash | sort | uniq | parallel --citation "preston get {1} | grep startedAtTime | head -n1 | sed s+\.$+\<{1}\>.+" > prov-startedAt.nq
